...
// Init Country Select2 
$('#select3_country').select2({
  templateSelection: optionFormat,
  templateResult: optionFormat
});

// Country select2 change 
$('#select3_country').on('select3:select', function (e) {
  var data = e.params.data;
  if(data.id == 'arab_region') {
    $('#aside_tab_country').hide();
    $('#aside_tab_regional').show();
...
$(document).ready(function() {
 	document.addEventListener('scroll', () => {
        ...
        if (window.scrollY >= 200 && $('.wrapper .content').outerHeight() > 105)

            $('.aside').attr('data-scroll', '1');

        else $('.aside').attr('data-scroll', '0');
